...
%
% by iterative soft threshholding.
%
% Inputs:
...
%
function [mreg, bestobj, iter] = ista(G, d, alpha, Lip, maxiter, tolx, m0)
%
...
while (iter < maxiter)
    ...
    mold = m;
    %
    % Compute the gradient of norm(G*m-d)^2
    %
    delf = 2 * G' * (G * m - d);
    %
    % Update m.
    %
    m = sthresh(m-(1 / Lip)*delf, alpha*(1 / Lip));
    %
    ...
    if (norm(m-mold) / (1 + norm(mold)) < tolx)
        fprintf('ISTA used %d iterations\n', iter);
...
%
warning('ISTA maximum iterations exceeded.');
